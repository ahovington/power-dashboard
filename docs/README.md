# Documentation Index

## Getting Started
- [README](../README.md) - Project overview and quick start

## Architecture & Design
- [Architecture Design](./architecture/DESIGN.md) - System design and patterns
- [Decision Records](./architecture/ADR.md) - Architecture decision records

## Development
- [Development Guide](./DEVELOPMENT.md) - Setup and development workflow
- [Testing Strategy](./TESTING.md) - Test-driven development approach

## API & Integration
- [API Integration Guide](./api/INTEGRATION.md) - How to add new providers
- [Enphase Integration](./api/ENPHASE.md) - Enphase-specific documentation
- [API Endpoints](./api/ENDPOINTS.md) - Backend API reference

## Database
- [Schema Documentation](./database/SCHEMA.md) - Database tables and relationships
- [Migrations](./database/MIGRATIONS.md) - Database migration guide

## Deployment
- [Docker Setup](./DOCKER.md) - Docker and compose configuration
- [Production Deployment](./DEPLOYMENT.md) - Production considerations